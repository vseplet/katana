...
var webrtcAPI = func() *webrtc.API {
	...
	// Пейсер ВКЛЮЧЁН — сглаживает залпы кейфреймов (см. pacer.go: фиксированная
	// скорость 6 Мбит, мелкие кадры проходят мгновенно, только всплеск кейфрейма
	// растягивается). Именно кейфрейм-залпы давали пики потерь 94% на шейпленном
	// канале. Прошлая беда пейсера (фантомные потери) была от привязки к битрейту —
	// теперь скорость фиксированная. KATANA_NO_PACER=1 выключает для A/B.
	if os.Getenv("KATANA_NO_PACER") != "1" {
		ir.Add(pacerFactory{})
	}

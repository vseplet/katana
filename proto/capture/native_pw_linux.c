...
struct nstate {
	...
	// энкод-поток (condvar). Энкодим ПО ПРИХОДУ кадра (фазовая синхронизация с
	// компоузитором → нет джаддера свободного таймера), но не в RT-потоке
	// PipeWire (иначе overload). Rate-cap (min-интервал) страхует от захлёба GPU,
	// если компоузитор проигнорит согласованный fps и сыпет damage-кадры быстрее.
	pthread_mutex_t mtx;
	pthread_cond_t cond;
...
// энкод-поток. Никакого энкода здесь — иначе пропуск RT-дедлайна → overload.
// Вычёрпываем всю очередь, оставляя последний буфер (не энкодим устаревшие).
...
// свежий кадр — то есть энкодит по приходу кадра (фаза синхронна с компоузитором,
// нет джаддера свободного таймера). Rate-cap: если с прошлого энкода прошло меньше
// interval, кадр пропускаем — гарантия ≤ fps, GPU не захлебнётся.
static void *enc_thread_fn(void *arg)
{
	...
	for (;;) {
		...
		// Rate-cap: не чаще одного энкода за interval (страховка от overload,
		// если компоузитор сыпет damage-кадры быстрее согласованного fps).
		long long t0 = now_ns();
		if (last_enc != 0 && t0 - last_enc < interval)
			continue;
		last_enc = t0;
...
int katana_native_start(katana_native_cfg cfg)
{
	...
	params[0] = spa_pod_builder_add_object(&pb,
		...
		SPA_FORMAT_VIDEO_framerate, SPA_POD_CHOICE_RANGE_Fraction(
			...
			&SPA_FRACTION(cfg.fps > 0 ? cfg.fps : 30, 1))); // max = target: компоузитор сам режет поставку
	...
	// Будим энкод-поток из condvar-ожидания и ждём выхода.
	pthread_mutex_lock(&S.mtx);
	S.stop_enc = 1;
	pthread_cond_signal(&S.cond);
	pthread_mutex_unlock(&S.mtx);

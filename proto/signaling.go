...
//
// Дебаунс — защита от IDR-шторма: на WAN (~300мс) зритель шлёт следующий PLI раньше,
// чем долетит и декодируется IDR от предыдущего; при слишком коротком дебаунсе каждый
// PLI рождает новый IDR в уже забитый канал — спираль (в native out видна горбами над
// потолком CBR). Секунда достаточна, чтобы один IDR дошёл и разорвал петлю.
//
// Снижаем до 350мс (≈1 RTT) ТОЛЬКО когда активный энкодер локализует потери (AMF:
// кейфрейм порезан на слайсы + intra-refresh, компактнее) — тогда шторм слабее, а
// восстановление после ПОВРЕЖДЁННОГО кейфрейма идёт через ~350мс, а не через секунду
// → фриз короче в ~3 раза. Для монолитных IDR (MF-фолбэк на не-AMD, mac VideoToolbox,
// linux VAAPI) короткий дебаунс вернул бы шторм — им прежняя секунда. Флаг приезжает
// со Stream от capture. Страховка stormCut ниже остаётся всем.
func (h *hub) requestKeyframe() {
	...
	h.mu.Lock()
	now := time.Now()
	// h.str читаем под h.mu (та же блокировка, что ниже). Локализует ли энкодер потери
	// (AMF/слайсы) — тогда дебаунс короче; env-override, если задан, старше всего.
	debounce := time.Second
	if h.str != nil && h.str.lossLocalizedEnc() {
		debounce = 350 * time.Millisecond
	}
	if envDebounceMs >= 0 {
		debounce = time.Duration(envDebounceMs) * time.Millisecond
	}
...
func (h *hub) onLoss(lost float64) {
	...
	switch {
	case lost > 0.80:
		// Абсурдные потери (>80%) — это НЕ перегрузка канала (реальная так чисто не
		// бьёт), а артефакт реконнекта/устаревшего потока: зритель отвалился/пере-
		// подключается и рапортует «всё потеряно». Реагировать = запустить петлю
		// смерти (битрейт в пол → 200kbps → фриз → реконнект → снова «96%»). Игнор.
		h.cleanTicks = 0
	case lost > 0.20: // реальная катастрофа — режем вдвое
		cur = cur / 2
		h.cleanTicks = 0
	...
	// Пол битрейта: ниже ~1000 kbps смотреть всё равно нельзя, а спад туда сам
	// запускает петлю смерти (непросматриваемо → фриз → реконнект → «потери» →
	// ещё спад). Держим поток на просматриваемом уровне даже под потерями — мелкие
	// хиккапы лучше, чем полный обвал. Пол не выше потолка настроек.
	floor := 1000
...
var webrtcAPI = func() *webrtc.API {
	...
	// Пейсер ВЫКЛЮЧЕН. Практика: с ним зритель видит RR loss 43-82% и очередь 150
	// пакетов (задержка читается как потери), без него — чистые lost=0, dec=60.
	// Любая его версия (привязанная к битрейту ИЛИ фиксированная) на этом канале
	// копит очередь и даёт фантомные потери. KATANA_PACER=1 — только для экспериментов.

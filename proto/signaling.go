...
//
// Дебаунс ~1 RTT — защита от IDR-шторма: на WAN (~300мс) зритель шлёт следующий PLI
// раньше, чем долетит и декодируется IDR от предыдущего; при слишком коротком дебаунсе
// каждый PLI рождает новый IDR в уже забитый канал — спираль (в native out видна
// горбами над потолком CBR). Раньше стоял 1с под жирные монолитные MF-IDR; с AMF-
// энкодером кейфрейм порезан на слайсы и компактнее (High), поэтому шторм слабее и
// можно короче — 350мс (≈1 RTT): восстановление после ПОВРЕЖДЁННОГО кейфрейма идёт
// через ~350мс, а не через секунду, → фриз короче в ~3 раза. Страховка от шторма
// (stormCut ниже) остаётся. Keyframe общий — одного достаточно всем зрителям.
